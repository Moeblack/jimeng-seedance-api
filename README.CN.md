...
对于耗时较长的生成请求（尤其是视频），建议使用异步 API 以避免连接超时。提交后立即返回 `task_id`，随时可以查询任务进度和最终结果。
...
返回 `status`（pending | processing | completed | failed）、`progress`（0-100）以及 `result` 或 `error`。

**3. 获取所有任务列表**

- **GET** `/v1/async/tasks`

支持可选查询参数 `?status=pending|processing|completed|failed` 进行筛选。
...
本项目衍生自开源项目 [jimeng-api](https://github.com/iptag/jimeng-api)（基于 GPL-3.0 协议）。感谢原作者及开源社区提供的优秀底层架构与灵感。

本项目现已更名并作为独立分支（Hard Fork）进行演进与维护。
- 原核心架构版权归原项目作者所有。
- 本项目新增的特性（如异步任务流、UI 控制台、音频上传等定制化功能）由本仓库独立维护。

本项目继续严格遵循 **GPL-3.0 开源协议** 开放源代码，特此致谢。
